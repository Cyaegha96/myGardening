...
export function ContextMenuPlugin(): JSX.Element {
  ...
  const items = useMemo(() => {
    return [
      new NodeContextMenuOption(`링크 제거`, {
        $onSelect: () => {
      ...
      new NodeContextMenuOption(`잘라내기`, {
        $onSelect: () => {
      ...
      new NodeContextMenuOption(`붙혀넣기`, {
        $onSelect: () => {
      ...
      new NodeContextMenuOption(`일반 텍스트로 붙혀넣기`, {
        $onSelect: () => {
          navigator.clipboard.read().then(async function (...args) {
            const permission = await navigator.permissions.query({
              // @ts-expect-error These types are incorrect.
      ...
      new NodeContextMenuSeparator(),
      new NodeContextMenuOption(`노드 삭제`, {
        $onSelect: () => {
